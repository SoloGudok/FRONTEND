  ...
  const toggleSubscription = (subscription) => {
    setSelected((prev) => {
      ...
      if (isSelected) {
        axios
          .put(
            `${process.env.REACT_APP_API_URL}/api/v1/subscription/unselect/${subscription.id}`
          )
      ...
      if (prev.length < 3) {
        axios
          .post(process.env.REACT_APP_API_URL+"/api/v1/subscription/select", [
            ...prev.map((s) => s.id),
            subscription.id,
          ])
        ...
        <div className="w-full mt-4">
          ...
          <div className="step-container">
            ...
            <ul style={{ textAlign: "left" }}>
              ...
              <li>
                이미 사용중인 구독 상품을 포함해서 조합 상품을 구매할 수는
                없어요.
              </li>
              <li>
                중도 해지시에는 해지일까지는 사용할 수 있지만, 해당 월
                이용요금은 환불받을 수 없으니 양해 부탁드려요요
              </li>
              <li>
